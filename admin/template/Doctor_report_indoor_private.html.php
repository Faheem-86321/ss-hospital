...
?>
    <table id="example"  class="table table-centered table-striped table-bordered mb-0 toggle-circle">
        <thead>
            <tr>
                <th></th>
                <th colspan="4">Doctor-wise Case Summary<br><?php echo $_GET['date_from']." to ".$_GET['date_to']; ?></th>
                <th></th>
            </tr>
            <tr>
                <th>Sr No.</th>
    ...
    $fetch_data = "
        SELECT 
            ...
            ssh_dr_reg.Name, 
            COUNT(ssh_p_indoor.pi_id) AS numberofcase,
            SUM(ssh_p_indoor_doctors.D_Fee) AS fee,
            SUM(ssh_p_indoor.Paid) AS Paid
        FROM ssh_p_indoor 
        JOIN ssh_p_indoor_doctors ON ssh_p_indoor.pi_id = ssh_p_indoor_doctors.pi_id 
        JOIN ssh_dr_reg ON ssh_p_indoor_doctors.D_ID = ssh_dr_reg.D_ID 
        WHERE ssh_p_indoor_doctors.to_paid = '1' 
            AND ssh_p_indoor.admition_type = '0' 
            AND CONVERT(ssh_p_indoor.admit_date, DATE) BETWEEN '{$_GET['date_from']}' AND '{$_GET['date_to']}'
        GROUP BY ssh_p_indoor_doctors.D_ID
    ";

    $fetch_data_ex = mysqli_query($con, $fetch_data);
    foreach($fetch_data_ex as $row) {
        $total_patient += $row['numberofcase'];
        $total_payment += $row['Paid'];
        $total_doc_share += $row['fee'];
        $hospital_share += ($row['Paid'] - $row['fee']);
?>
    <tr>
        <td><?php echo $sr_no++; ?></td>
        <td><?php echo $row['Name']; ?></td>

        <input type="hidden" id="date_from" value="<?php echo $_GET['date_from']; ?>">
        <input type="hidden" id="date_to" value="<?php echo $_GET['date_to']; ?>">

        <td>
            <?php echo $row['numberofcase']; ?>
            <button class='btn btn-success ml-1' onclick='view_indoor_private(<?php echo $row['D_ID'] ?>,0);' style='padding: 4px 4px; float:right;'><i class='fa fa-eye'></i></button>
        </td>
...
<?php } else { ?>
    <div class="alert alert-success">Select Date Range to view Records !!</div>
<?php } ?>
